...
from src.delete import remove_old_hashtags
from src.maps import get_category_from_post
from src.maps import add_tag_map_links
...
from src.categories import get_hashtag_categories
...
from src.webapp_utils import get_content_warning_button
...
def store_hash_tags(base_dir: str, nickname: str, domain: str,
    ...
    if not post_json_object.get('id'):
        return
    if not isinstance(object_tags, list):
        return
    ...
    if event_category_str:
        if ', ' in event_category_str:
            ...
            event_category_list = [event_category_str]
        for tag_name_text in event_category_list:
            if '/' in tag_name_text:
                tag_names_list = tag_name_text.split('/')
            else:
